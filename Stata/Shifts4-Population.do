...
import delimited "rawdata/population.csv", varnames(1) clear

set varabbrev off

keep countryname countrycode yr1990 yr2000 yr2010
...
replace bplgroup = 15 if countryname == "Faroe Islands"
...
collapse (sum) Pop, by(bplgroup year)

reshape wide Pop, i(year) j(bplgroup)

forval val = 1/19 {
	cap confirm variable Pop`val'
	if _rc {
		gen Pop`val'=0
	}
	else {
		replace Pop`val'=0 if Pop`val'==.
	}
}
