...
import excel "C:rawdata\2020_Country_and_Territory_Ratings_and_Statuses_FIW1973-2020.xlsx", ///
	sheet("Country Ratings, Statuses ") cellrange(A3:EL208) firstrow clear

drop B-AZ DL-EL

loc year_local = 1990
loc loop_index = 1
foreach var of varlist BA-DK {
	if `loop_index' == 1 {
		ren `var' PR`year_local'
	}
	if `loop_index' == 2 {
		ren `var' CL`year_local'
	}
	if `loop_index' == 3 {
		ren `var' Status`year_local'
	}
	if `loop_index' < 3 {
		local loop_index = `loop_index' + 1
	}
	else {
		local loop_index = 1
		local year_local = `year_local' + 1
	}
}

keep A PR1990 PR2000 PR2010 CL1990 CL2000 CL2010 Status1990 Status2000 Status2010
ren A countryname

foreach var of varlist PR1990 CL1990 PR2000 CL2000 PR2010 CL2010 {
	replace `var' = "1" if (countryname == "Latvia" & `var' == "-") | (countryname == "Lithuania" & `var' == "-") | (countryname == "Estonia" & `var' == "-")
} 

foreach var of varlist Status1990 Status2000 Status2010 {
	replace `var' = "PF" if (countryname == "Latvia" & `var' == "-") | (countryname == "Lithuania" & `var' == "-") | (countryname == "Estonia" & `var' == "-")
}
...
destring CL* PR*, replace
collapse (mean) CL* PR* Status*, by(bplgroup)

reshape long PR CL Status, i(bplgroup) j(year)
reshape wide PR CL Status, i(year) j(bplgroup)

save data/FreedomHouse, replace
